...
object ReportsView:
  ...
  def apply(): Html[Msg] =
    div()(
      div(cls := "mb-8")(
        h1(cls := "text-3xl font-bold text-gray-100 mb-2")("Reports"),
        p(cls := "text-gray-400")("Generate and view detailed reports")
      ),
      ...
      div(cls := "card")(
        div(cls := "flex items-center justify-between mb-6")(
          h2(cls := "text-xl font-semibold text-gray-100")("Recent Reports"),
          button(cls := "badge-primary")("Generate New")
        ),
        div(cls := "space-y-4")(
  ...
  private def statCard(label: String, value: String): Html[Msg] =
    div(cls := "card text-center")(
      div(cls := "text-3xl font-bold text-gray-100 mb-2")(value),
      div(cls := "text-gray-400 text-sm")(label)
    )

  private def reportItem(title: String, period: String, status: String, badgeClass: String): Html[Msg] =
    div(cls := "flex items-center justify-between p-4 rounded-lg bg-gray-800/30 hover:bg-gray-800/50 transition-colors")(
      div(cls := "flex items-center gap-4")(
        div(cls := "w-10 h-10 rounded-lg bg-blue-500/10 flex items-center justify-center")(
          svg(cls := "w-5 h-5 text-blue-400", attribute("fill", "none"), attribute("stroke", "currentColor"), attribute("viewBox", "0 0 24 24"))(
        ...
        div()(
          div(cls := "font-medium text-gray-200")(title),
          div(cls := "text-sm text-gray-400")(period)
        )
      ),
      div(cls := "flex items-center gap-3")(
        span(cls := badgeClass)(status),
        button(cls := "text-gray-400 hover:text-gray-200")(
